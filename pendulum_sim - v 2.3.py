...
def build_mass_matrix(theta, lengths, masses, n):
    M = np.zeros((n, n))
    cum_mass = [sum(masses[k:]) for k in range(n)]
    for p in range(n):
        for q in range(n):
            k = max(p, q)
            M[p, q] = lengths[p] * lengths[q] * np.cos(theta[p] - theta[q]) * cum_mass[k]
    return M

def build_force(theta, omega, lengths, masses, damping, g, n):
    f = np.zeros(n)
    cum_mass = [sum(masses[k:]) for k in range(n)]
    for p in range(n):
        coupling = 0.0
        for q in range(n):
            if q == p: continue
            k = max(p, q)
            coupling += lengths[q] * np.sin(theta[p] - theta[q]) * omega[q]**2 * cum_mass[k]
        f[p] = -lengths[p] * (coupling + g * np.sin(theta[p]) * cum_mass[p])
        f[p] -= damping * omega[p]  # 阻尼项
    return f

def n_pendulum_derivatives(t, state, lengths, masses, damping, g):
    n = len(lengths)
    theta, omega = state[:n], state[n:]
    M = build_mass_matrix(theta, lengths, masses, n)
    f = build_force(theta, omega, lengths, masses, damping, g, n)
    alpha = np.linalg.solve(M, f)
    dst = np.zeros(2 * n)
    dst[:n], dst[n:] = omega, alpha
    return dst

def rk4_step(f, state, t, dt, lengths, masses, damping, g):
    k1 = f(t, state, lengths, masses, damping, g)
    k2 = f(t + dt/2, state + dt/2*k1, lengths, masses, damping, g)
    k3 = f(t + dt/2, state + dt/2*k2, lengths, masses, damping, g)
    k4 = f(t + dt, state + dt*k3, lengths, masses, damping, g)
...
class PendulumSim:
    def __init__(self):
        ...
        self._smooth_xlim = None        # 缩放平滑状态
        self._smooth_ylim = None
        ...
        self.paused = False
        self._blit_stale = True   # blit=True 时标记是否需要刷新背景缓存
        self.anim = None
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        # ═══════════════ MASS ═══════════════
        sec_label('MASS', row(idx))
        self.mass_sl = []
        for i in range(7):
            a = wdg(sx, row(idx), sw_full)
            v = self.masses[0] if i == 0 else 1.0
            s = Slider(a, f'M{i+1}', 0.1, 10.0, valinit=v, valstep=0.1, color=COLORS[i])
            s.on_changed(self._on_mass_change)
            s.ax.set_visible(i < self.n)
            self.mass_sl.append(s)
            idx += 1
        idx += 1  # spacer
        ...
        # Row: Damping
        a = wdg(sx, row(idx), sw_full)
        self.sl_damping = Slider(a, 'Damping (b)', 0.0, 5.0, valinit=self.damping,
                                 valstep=0.01, color='#e74c3c')
        self.sl_damping.on_changed(self._on_damping_change)
        idx += 1
    ...
    def _init_physics(self):
        ...
        self._smooth_xlim = None
        self._smooth_ylim = None
        self._rescale()

    def _rescale(self, xs=None, ys=None):
        """缩放视图。返回 True 表示视图范围发生了可观变化（>0.2%）."""
        ...
        if self.auto_zoom and self.n > 0:
            ...
            target = (x_min - margin_x, x_max + margin_x,
            ...
            smooth = 0.25
            if self._smooth_xlim is None:
                self._smooth_xlim = (target[0], target[1])
                self._smooth_ylim = (target[2], target[3])
            else:
                self._smooth_xlim = (
                    self._smooth_xlim[0] + smooth * (target[0] - self._smooth_xlim[0]),
                    self._smooth_xlim[1] + smooth * (target[1] - self._smooth_xlim[1]),
                )
                self._smooth_ylim = (
                    self._smooth_ylim[0] + smooth * (target[2] - self._smooth_ylim[0]),
                    self._smooth_ylim[1] + smooth * (target[3] - self._smooth_ylim[1]),
                )
        ...
        # 检测 limits 是否真有可观变化（阈值 0.2%）
        new_xlim = self.ax.get_xlim()
        new_ylim = self.ax.get_ylim()
        rx = max(new_xlim[1] - new_xlim[0], 1.0)
        ry = max(new_ylim[1] - new_ylim[0], 1.0)
        eps = 0.002
        if (abs(new_xlim[0] - old_xlim[0]) > eps * rx or
            abs(new_xlim[1] - old_xlim[1]) > eps * rx or
            abs(new_ylim[0] - old_ylim[0]) > eps * ry or
            abs(new_ylim[1] - old_ylim[1]) > eps * ry):
            return True
        return False
    ...
    def _on_n_change(self, val):
        ...
        while len(self.masses) < self.n:
            self.masses.append(1.0)
        self.masses = self.masses[:self.n]

        # 自适应轨迹长度：多摆时缩短以减少渲染开销
        new_len = max(40, 120 - (self.n - 1) * 15)
        if new_len != self.trail_length:
            self.trail_length = new_len
        ...
        for i, sl in enumerate(self.mass_sl):
            vis = i < self.n
            sl.ax.set_visible(vis)
            if vis and i < len(self.masses):
                sl.set_val(self.masses[i])
    ...
    def _on_mass_change(self, val):
        for i, sl in enumerate(self.mass_sl):
            if i < self.n:
                self.masses[i] = sl.val
        self._rescale()

    def _on_damping_change(self, val):
        self.damping = val
        self.fig.canvas.draw_idle()
    ...
    def _apply_config(self, cfg):
        ...
        self.masses = list(cfg.get('masses', [1.0]))
        self.damping = cfg.get('damping', 0.0)
        ...
        self.sl_damping.set_val(self.damping)
    ...
    def _update_pendulum_inplace(self, xs, ys):
        """原地更新摆锤 artist 坐标（使用已计算的 xs/ys，不重复计算）."""
        if self.n == 0:
            return
        px, py = 0.0, 0.0
        for i in range(self.n):
    ...
    def _get_artists(self):
        """返回待刷新 artist 列表（用于 blit=True 局部更新）."""
        arts = []
        arts.extend(self.rod_lines)
        arts.extend(self.bob_patches)
        arts.extend(self.trail_artists)
        if self.pivot:
            arts.append(self.pivot)
        return arts
    ...
    def _update(self, frame):
        try:
            ...
            if not self.paused:
                ...
                for _ in range(n_steps):
                    self.state = rk4_step(n_pendulum_derivatives,
                                          ...
                                          self.lengths[:self.n],
                                          self.masses[:self.n], self.damping, self.g)
                    self.t += self.dt
                theta = self.state[:self.n]

            # NaN/Inf 检测：混沌系统长时间运行可能数值溢出
            if np.any(np.isnan(self.state)) or np.any(np.isinf(self.state)):
                self._init_physics()
                self._init_artists()
                self.paused = True
                self.pause_btn.label.set_text('▶ Continue')
                print('N-Pendulum: Physics instability detected, auto-reset.')
                self._blit_stale = True
                self.fig.canvas.draw()
            ...
            if self._rescale(xs, ys):
                self._blit_stale = True
            if self._update_trails():
                self._blit_stale = True
            if self._blit_stale:
                self.fig.canvas.draw()
